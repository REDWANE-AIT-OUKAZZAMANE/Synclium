...
/** v1: Serve the repo's example invoices so the demo has one-click samples. */
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { samples },
      { headers: { "X-API-Version": "1.0", "X-API-Deprecated": "false" } },
    );
  } catch (e) {
    return NextResponse.json(
      { samples: {}, error: (e as Error).message },
