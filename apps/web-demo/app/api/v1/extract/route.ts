...
/**
 * v1 GET: Query current remaining quota and authentication tier without consuming quota
 */
export async function GET(req: Request) {
  try {
    ...
    headers.set("X-API-Deprecated", "false");
...
/**
 * v1 POST: Stateless AI extraction — verifies Turnstile challenge, authenticates session,
...
export async function POST(req: Request) {
  try {
    ...
    rateLimitHeaders.set("X-API-Version", "1.0");
    rateLimitHeaders.set("X-API-Deprecated", "false");
    ...
    return NextResponse.json({ error: (e as Error).message }, { status: 400, headers: { "X-API-Version": "1.0" } });
  }
}
